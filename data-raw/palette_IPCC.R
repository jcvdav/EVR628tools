...
colors <- dplyr::tibble(
  ...
  dplyr::mutate(rgb = purrr::map(links$link,
                                 readr::read_table,
                                 col_names = c("R", "G", "B")),
                hex = purrr::map(rgb, rgb2hex))


temp_div <- colors$hex[colors$name == "temp_div"]
temp_seq <- colors$hex[colors$name == "temp_seq"]
prec_div <- colors$hex[colors$name == "prec_div"]
prec_seq <- colors$hex[colors$name == "prec_seq"]
wind_div <- colors$hex[colors$name == "wind_div"]
wind_seq <- colors$hex[colors$name == "wind_seq"]
cryo_div <- colors$hex[colors$name == "cryo_div"]
cryo_seq <- colors$hex[colors$name == "cryo_seq"]
